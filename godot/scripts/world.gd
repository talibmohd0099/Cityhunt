...
const FOG_BASE := Color(0.098, 0.098, 0.112)   # linear, like the browser version
const FOG_END := 150.0
...
func _environment() -> void:
	...
	# three.js FogExp2 (density 0.0115) is matched closely by a smoothstep depth fog ending at 150 m
...
func _city_meshes() -> void:
	...
	var mats := {
		...
		"mark": shader_mat("vcolor", {rough = 0.45, metal = 0.0, tint = Vector3(0.54, 0.53, 0.5)}),
		...
		"car_paint": shader_mat("vcolor", {rough = 0.3, metal = 0.45, coat = 1.0}),
...
# ---------------------------------------------------------------- vehicles (uploaded 3D models)
func _vehicles() -> void:
	var slots: Array = data.vslots
	if slots.is_empty():
		return
	var mats := {
		"paint": _std(Color(1, 1, 1), 0.3, 0.45, 1.0),
		"glass": _std(Color(0x06 / 255.0, 0x09 / 255.0, 0x0c / 255.0), 0.04, 0.8, 1.0),
	...
	(mats.paint as StandardMaterial3D).vertex_color_use_as_albedo = true
	(mats.tail as StandardMaterial3D).emission_enabled = true
	(mats.tail as StandardMaterial3D).emission = Color(0x22 / 255.0, 0, 0)
	var ok := true
	var by_model := {}
	for s in slots:
		if not by_model.has(s.vm):
			by_model[s.vm] = []
		by_model[s.vm].append(s)
	for vm in by_model:
		var path := "res://assets/vehicles/%s.glb" % vm
		if not ResourceLoader.exists(path):
			ok = false
			continue
		var scn: Node3D = load(path).instantiate()
		var list: Array = by_model[vm]
		for n in scn.find_children("*", "MeshInstance3D", true, false):
			var mi := n as MeshInstance3D
			var mm := MultiMesh.new()
			mm.transform_format = MultiMesh.TRANSFORM_3D
			var role := String(mi.name)
			mm.use_colors = role == "paint"
			mm.mesh = mi.mesh
			mm.instance_count = list.size()
			var local := mi.global_transform if mi.is_inside_tree() else _node_xform(mi, scn)
			for i in list.size():
				var s: Dictionary = list[i]
				mm.set_instance_transform(i, Transform3D(Basis(Vector3.UP, s.ry), Vector3(s.x, 0, s.z)) * local)
				if role == "paint":
					mm.set_instance_color(i, Color(s.col[0], s.col[1], s.col[2]))
			var mmi := MultiMeshInstance3D.new()
			mmi.multimesh = mm
			mmi.material_override = mats.get(role, mats.trim)
			mmi.name = "Vehicle_%s_%s" % [vm, role]
			add_child(mmi)
		scn.free()
	if ok:
		for m in fb_meshes:
